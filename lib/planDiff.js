...
// Render a diff as a compact plain-text brief for the comparison commentary prompt.
// Deterministic; the model gets the facts and only writes the prose around them.
const money = (n) => (n == null ? "—" : "$" + Number(n).toLocaleString("en-US"));
const ratePct = (r) => (r == null ? "—" : (Math.round(Number(r) * 1000) / 10) + "%");
const tiersStr = (ts) => (Array.isArray(ts) && ts.length ? ts.map((t) => ratePct(t.rate)).join("/") : "—");
const PAY_LABEL = { base_salary: "Base salary", ote: "OTE", target_variable: "Target variable" };

export function formatDiffBrief(diff) {
  if (!diff || !diff.hasPrior) return "No prior plan to compare.";
  const lines = [];
  lines.push(`Comparing FY${diff.priorYear ?? "?"} (prior) -> FY${diff.currentYear ?? "?"} (current).`);
  if (diff.pay.length) {
    lines.push("", "Pay changes:");
    for (const p of diff.pay) lines.push(`- ${PAY_LABEL[p.field] || p.field}: ${money(p.from)} -> ${money(p.to)}`);
  }
  if (diff.components.length) {
    lines.push("", "Component changes:");
    for (const c of diff.components) {
      if (c.change === "added") {
        lines.push(`- ${c.name} (ADDED this year): weight ${c.weight.to == null ? "—" : c.weight.to + "%"}, base rate ${ratePct(c.rate.to)}, quota ${money(c.quota.to)}, tiers ${tiersStr(c.tiers.to)}`);
      } else if (c.change === "removed") {
        lines.push(`- ${c.name} (REMOVED this year): was weight ${c.weight.from == null ? "—" : c.weight.from + "%"}, base rate ${ratePct(c.rate.from)}, quota ${money(c.quota.from)}`);
      } else {
        const parts = [];
        if (c.weight) parts.push(`weight ${c.weight.from}% -> ${c.weight.to}%`);
        if (c.rate) parts.push(`base rate ${ratePct(c.rate.from)} -> ${ratePct(c.rate.to)}`);
        if (c.quota) parts.push(`quota ${money(c.quota.from)} -> ${money(c.quota.to)}`);
        if (c.tiers) parts.push(`tiers ${tiersStr(c.tiers.from)} -> ${tiersStr(c.tiers.to)}`);
        lines.push(`- ${c.name} (changed): ${parts.join("; ")}`);
      }
    }
  }
  if (!diff.pay.length && !diff.components.length) lines.push("", "No material changes between the two plans.");
  return lines.join("\n");
}
