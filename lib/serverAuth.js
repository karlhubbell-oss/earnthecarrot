...
export async function resolveRepId(sql, identity, fallback = {}) {
  ...
  const existing = await sql`SELECT id FROM reps WHERE auth_user_id = ${identity.authUserId} LIMIT 1`;
  if (existing.length) {
  ...
  try {
    ...
    const again = await sql`SELECT id FROM reps WHERE auth_user_id = ${identity.authUserId} LIMIT 1`;
