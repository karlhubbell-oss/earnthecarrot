// Shared reshaping: turn a compensation_plans row + its facts back into the plan
// object the screens expect (the inverse of api/save-plan). Used by get-plan for both
// the current plan and a specific plan fetched by id, so single- and multi-plan reads
// share one code path. Pure-ish: it runs the per-plan sub-queries against the passed
// sql client and returns { plan, coachTake }.

const num = (v) => (v == null ? null : Number(v));
const dstr = (v) => (v == null ? null : String(v).slice(0, 10));

// Build the plan object from already-fetched rows. No DB access.
export function reshapePlan(planRow, rep, facts, periods, docName) {
  const plan = {
    schema_version: "1.0",
    meta: {
      ...
      plan_id: planRow.id,
      plan_year: planRow.plan_year != null ? Number(planRow.plan_year) : null,
...
// Load + reshape a single plan row (fetches its rep, facts, periods, source doc).
// Returns { plan, coachTake }. The caller selects/authorizes planRow first.
export async function loadPlanFromRow(sql, planRow) {
  const repRows = await sql`SELECT * FROM reps WHERE id = ${planRow.rep_id} LIMIT 1`;
  const rep = repRows[0] || {};
  ...
  const plan = reshapePlan(planRow, rep, facts, periods, docName);
  const coachTake = planRow.coach_take ?? null;
  return { plan, coachTake };
}
