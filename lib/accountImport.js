...
// Some exports flag which accounts are already customers. Detect that column by
// best-guess header (like the name column) and map its value to customer/prospect.
// If there is no such column, everything defaults to prospect.
const STATUS_HEADERS = ["current client", "existing customer", "is customer", "is client", "customer status", "client status", "customer", "client"];
const CUSTOMER_VALUES = new Set(["yes", "y", "true", "t", "1", "customer", "client", "existing", "existing customer", "current", "current client"]);
...
function mapStatus(cell) {
  ...
  return CUSTOMER_VALUES.has(v) ? "customer" : "prospect";
}
...
export function buildImport(rows2d) {
  ...
  const statusIdx = detectStatusColumn(headers);
  ...
  dataRows.forEach((r, i) => {
    ...
    const customer_status = statusIdx >= 0 ? mapStatus(r[statusIdx]) : "prospect";
    accounts.push({ name: name || null, customer_status, fields });
  });

  // Report columns in file order, not just seen order.
  const columns = headers.filter((h) => columnsSeen.has(h));
  return { accounts, columns, nameColumn: headers[nameIdx], nameColumnGuessed: guessed, statusColumn: statusIdx >= 0 ? headers[statusIdx] : null, skipped, warnings };
}
